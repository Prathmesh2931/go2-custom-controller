...
public:
    Go2StandNode() : Node("go2_stand_node") {
        ...
        RCLCPP_INFO(this->get_logger(), "Standing Controller Active. Chattering filtered.");
    }

private:
    const std::vector<double> targets = {0.0, 0.67, -1.30, 0.0, 0.67, -1.30, 0.0, 0.67, -1.30, 0.0, 0.67, -1.30};
    
    // We are setting FF to 0.0. A high Kp will naturally overcome gravity.
    // This stops conflicting forces during the ramp up phase.
    ...
    // THE FIX:
    // Kp lowered slightly to reduce spring bounce.
    // Kd drastically reduced to eliminate the high-frequency velocity vibrations!
    double kp = 90.0; 
    double kd = 0.8;
    ...
    void control_loop() {
        ...
        // Calculate smooth trajectory ramp over 2.0 seconds
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            // Pure PD loop (Velocity chatter removed by low kd)
            double tau = (kp * (active_target - pos)) - (kd * vel) + (ff[i] * ramp);
